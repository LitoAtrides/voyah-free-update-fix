...
const scpFallbackMessage = "SFTP недоступен, использую SCP как запасной вариант."
...
func newTBoxFileClient(sshClient *ssh.Client) (tboxFileClient, error) {
	...
	if pingErr := scpClient.ping(); pingErr != nil {
		return nil, fmt.Errorf("ошибка инициализации sftp (%w); запасной переход на scp не удался: %w", err, pingErr)
	}
