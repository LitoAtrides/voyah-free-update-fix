...
var errContextJSONNotFound = errors.New("context.json не найден на T-Box")
...
func downloadContextDB(client tboxFileClient) error {
	...
	if ok := fileExistsOnTBox(client, contextDBPath); !ok {
		return fmt.Errorf("файл %s не найден", contextDBPath)
	}
	...
	if journalExists {
		return errors.New("журнал БД существует, файл занят. Попробуйте позже")
	}
...
func backup(withContextJSON bool) (string, error) {
	if err := os.MkdirAll(backupDir, backupDirMode); err != nil {
		return "", fmt.Errorf("ошибка создания каталога backup %s: %w", backupDir, err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("ошибка создания backup-архива %s: %w", archivePath, err)
	}
	defer zipFile.Close()
	...
	if err := zw.Close(); err != nil {
		return "", fmt.Errorf("ошибка закрытия backup-архива %s: %w", archivePath, err)
	}
...
func addFileToZip(zw *zip.Writer, localPath, zipName, archivePath string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("ошибка создания zip-записи %s: %w", zipName, err)
	}

	if _, err := io.Copy(entry, src); err != nil {
		_ = src.Close()

		return fmt.Errorf("ошибка записи backup-архива %s: %w", archivePath, err)
	}

	if err := src.Close(); err != nil {
		return fmt.Errorf("ошибка закрытия локального файла %s: %w", localPath, err)
	}
...
func rebootTBox(sshClient *ssh.Client) error {
	...
	if err := session.Start(cmd); err != nil {
		return fmt.Errorf("ошибка выполнения команды перезагрузки: %w", err)
	}
