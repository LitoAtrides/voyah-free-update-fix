...
Если фикс не нужен, утилита покажет сценарий OTA:
- `1. Сделать бэкап`
- `2. Перезапустить OTA`
- `3. Обновить expire_time`
- `4. Перезагрузить T-Box`
- `5. Показать готовность OTA`
- `6. Выйти`
- пункт `Сделать бэкап` создаёт архив текущего состояния `context.db` и, если он есть, `context.json`
- пункт `Перезапустить OTA` предлагает выбрать локальный backup-архив, восстанавливает из него OTA-структуру, сбрасывает прогресс загрузки и затем предлагает обновить `expire_time`
- при выборе обновления нужно ввести дату и время в формате `ДД.ММ.ГГГГ ЧЧ:ММ`, затем подтвердить изменение через `start`
- после подтверждения утилита обновит `expire_time` в `context.db` на T-Box и покажет путь к созданному backup
- после команды перезагрузки утилита будет ждать, пока T-Box снова появится в сети, и затем продолжит работу; в консоли показывается номер попытки

Что добавлено в этой форк-версии:
- отдельный интерактивный сценарий `Сделать бэкап`
- интерактивный сценарий `Перезапустить OTA`
- обновление `expire_time` из основного меню без отдельного запуска утилиты
- подтверждение изменений через `start`
- ожидание переподключения T-Box после `reboot`
- вывод статуса сценария OTA перед выбором действия
- русифицированные сообщения CLI
...
Дополнительно в сценарии OTA выводится:

- `Сделать бэкап` — создаёт архив текущего состояния `context.db` и `context.json` при наличии;
- `Перезапустить OTA` — восстанавливает согласованный OTA-снимок из локального backup-архива;
- `Обновить expire_time` — меняет срок действия OTA-задачи в `context.db`;
- `Перезагрузить T-Box` — перезагружает T-Box и ждёт повторного подключения;
- `Показать готовность OTA` — печатает актуальное состояние без изменений.
...
## Утилита для `expire_time`

Если нужно точечно изменить `expire_time` внутри локального `context.db`, можно собрать отдельную команду:

```bash
go build -o set-expire-time ./cmd/set-expire-time
```

Интерактивный запуск:

```bash
./set-expire-time --db /path/to/context.db
```

Команда спросит:

- дату и время в одной строке, предлагая по умолчанию текущее время + 15 минут
- затем попросит подтверждение, и только после этого применит изменение

Подтверждение выглядит так:

```text
=== Сценарий OTA ===
expire_time: 30.07.2025 10:44:46 (просрочен)
overall_state: stage=Download state=Process
download_state: stage=Retrive Packages progress=19%
Сценарий OTA:
1. Сделать бэкап
2. Перезапустить OTA
3. Обновить expire_time
...
6. Выйти
Выберите действие [1]:
Выбрано: 30.07.2025 10:44:05
Можно обновлять expire_time.
Подтвердите обновление expire_time. Введите 'start':
Перезагрузить T-Box. Введите 'start':
```

По умолчанию команда:

- делает локальную backup-копию `context.db.bak.YYYYMMDD_HHMMSS`
- проверяет целостность SQLite-базы
- обновляет только `ota_task.taskData -> expire_time`
- печатает старое и новое значение в Unix-времени и в человекочитаемом виде
- после обновления может перезагрузить T-Box, если вы подтвердите это отдельно

Если backup не нужен, можно отключить его флагом `--backup=false`.
